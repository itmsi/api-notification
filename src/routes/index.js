...
// Initialize Firebase Admin SDK on startup (lazy-safe, no crash if unconfigured)
try {
  initFirebase()
} catch (err) {
...
if (isSwaggerEnabled()) {
  ...
  // Alias: /swagger → /documentation (for compatibility with requirement spec)
  router.use('/swagger', swaggerUi.serve)
  router.get('/swagger', swaggerUi.setup(index, { isExplorer: false }))
}
